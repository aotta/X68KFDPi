...
void loop() {
  ...
  while(1) {
    ...
    BlinkCounter++;
    if (BlinkCounter>=69999) BlinkCounter=0;
  ...
  if (!digitalRead(OPT0)) {
    ...
    if (!digitalRead(Eject)) {   // era Eject
      //DF0=0;
      Serial.println("Ej 0-");
    }      
 
    if (!digitalRead(EjectMSK)) {
      digitalWrite(LED_FD0_RED,HIGH);
    ...
    if (!DF0) {
         ...
         digitalWrite(FDDINT,HIGH);  // not remove!! era HIGH
